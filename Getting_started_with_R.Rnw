...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{My tips about the global options}
5. I like to put the \texttt{source} and the \texttt{console} panes side-by-side to give them more vertical space on the screen
\begin{center}
...
\item start your script with \verb`rm(list = ls())`
...
\pause
\begin{itemize}[<+->]
\item \verb+->+ works too (if you swith the lefthand side and the righthand side)
\item ``\texttt{\_}'' and ``\texttt{.}'' are OK but avoid spaces \& other weird characters in names
\item names are case sensitive
\item use \texttt{<-} for assignments, not \texttt{=}
\end{itemize}
\end{frame}
...
\end{center}
\vfill
\textbf{NB:} this is failing on some 1.2.xxx versions of RStudio
\end{frame}
...
\vspace{1em}
\begin{itemize}
\item each column is a variable, usually corresponding to a \underline{vector} (a serie of elements of 1 type)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{the \texttt{\{tidyverse\}}}
...
\pause
\vfill
{\bf Note:} \verb`summarise()` only accepts functions that return a vector of length 1

{\bf Tip:} \verb`summarise()` ungroup automatically
\end{frame}
...
\begin{itemize}[<+->]
  ...
  \item A set of \textbf{aes}thetic mappings describing which variables are mapped to which aesthetics
  \item Layers made up of geometric elements (or \textbf{geom}s)
...
Overridding the \verb`geom_*()` default statistic is sometimes useful:\\
\vspace{1em}
<<layer_ecdf>>=
ggplot(iris) +
...
\item \#rstats
\item \#tidyverse
\item @hadleywickham ($>$92K followers)
\item @dataandme ($>$40K)%Mara Averick
\item @drob ($>$39K)%David Robinson
\item @rdpeng ($>$38K)%Roger Peng
\item @JennyBryan ($>$27K)
\item @statgarrett ($>$17K)%Garrett Grolemund
\item @RStudio ($>$84K)
\item @Rbloggers ($>$74K)
\item @RLangTip ($>$64K)% one \r \ tip per day
\item @rstudiotips ($>$53K)
\item @R\_Programming ($>$31K)
\item @WeAreRLadies \& @RLadiesGlobal (2$\times>$13K)
\item @rstatstweet \& @rstats4ds (bot like)
\item @rdataberlin (new!)
\end{scriptsize}
\end{itemize}
\end{itemize}
\end{columns}
