...
\usepackage{upquote} % for backtick within verbatim
...
\date[February 2026]{\small February 2026}%
...
\item rich (\url{https://rdrr.io};\url{https://r-universe.dev}) %\url{https://www.rdocumentation.org}
...
\begin{frame}{Installation steps}
  ...
  \end{enumerate}
  \vspace{2cm}
  If you have an IZW-owned computer, you may have to use the software kiosk instead:\\ \url{https://swkiosk/}
\end{frame}
...
\item integrated with several packages developed by Posit (e.g. \verb`{readr}`)
...
\large{10 most useful items:}
  \begin{itemize}
    ...
    \item \texttt{Session $\rightarrow$ Restart R}
...
\begin{itemize}
\item \texttt{Rtools} if you use Windows (on the IZW software kiosk, or at \url{https://cran.r-project.org/bin/windows/Rtools})
...
\item dominant paradigm for last generations of \r \ users
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\pause
\vspace{1em}
Keep rows per group using \texttt{filter(..., .by = xxx)}:\\
<<filter details2>>=
iris_tbl |> filter(Sepal.Length == max(Sepal.Length), .by = Species)
@
\pause
\vfill
{\bf Tip:} using \texttt{.by} is safer than \verb`group_by()` because the grouping is not persistent
\end{frame}
...
@
\pause
Create a column based on values per group using \texttt{mutate(..., .by = xxx)}:\\
<<mutate details2>>=
iris_tbl |> mutate(Sepal.Length.max = max(Sepal.Length), .by = Species)
@
\pause
\vfill
...
@
\pause
Compute summary statistics per group using \texttt{summarise(..., .by = xxx)}:\\
<<summarise details2>>=
iris_tbl |> summarise(Sepal.Length.max = max(Sepal.Length), N = n(), .by = Species)
@
\pause
\vfill
{\bf Tip:} \verb`summarise()` can only return one row per group;  \verb`reframe()` is more flexible
\end{frame}
...
\item Google Sheets $\rightarrow$ \verb`{googlesheets4}`
...
ggplot(iris) +
  aes(x = Sepal.Length, y = Sepal.Width, colour = Species) +
  geom_point() + 
  coord_transform(y = "log10", ylim = c(1.5, 5))
@
\end{frame}
...
\begin{columns}
\column{0.3\linewidth}
<<theme_void, out.width = "\\linewidth">>=
myplot +
  theme_void()
@
\column{0.3\linewidth}
<<theme_dark, out.width = "\\linewidth">>=
myplot +
  theme_dark()
@
\column{0.3\linewidth}
<<theme_minimal, out.width = "\\linewidth">>=
myplot +
...
do it manually, or try \verb+{installr}+ or \verb+{updater}+ on Windows
...
\item online courses: e.g. \url{https://www.coursera.org}, \url{https://www.mondegoscience.com}
\end{itemize}
\vfill
\pause
...
\item R-Ladies (\url{https://rladies.org})
...
\begin{itemize}
\begin{scriptsize}
\item The R Project for Statistical Computing (group)
\end{scriptsize}
\end{itemize}
\end{itemize}
\end{columns}
